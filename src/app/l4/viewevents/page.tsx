...
export default function EventTable() {
  const [events, setEvents] = useState<Event[]>([]); // Type events as an array of Event objects

  useEffect(() => {
    const guruName = sessionStorage.getItem("guru");
    const decodedGuruName = decodeURIComponent(guruName || ""); // Default to empty string if guruName is null
    ...
    const fetchEvents = async () => {
      try {
        ...
        const data: Event[] = await response.json(); // Type the response data
