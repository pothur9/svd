...
export default function Dashboard(): JSX.Element {
  ...
  const handlePhotoUpload = async (file: File) => {
    ...
    try {
      ...
      const updateRes = await fetch(`/api/l2/update-profile/${userId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ imageUrl: photoUrl }),
      });
      if (!updateRes.ok) throw new Error('Failed to update profile');
      const refreshed = await fetch(`/api/l2/dashboard/${userId}?timestamp=${Date.now()}`, { cache: 'no-store' });
      if (refreshed.ok) {
        ...
        <div style={{ background: '#f8fafc', minHeight: '100vh', paddingBottom: '2rem' }}>
          ...
          <div style={{ padding: '16px 16px 0' }}>
            ...
            {(!userData.imageUrl || !userData.imageUrl.startsWith('http')) && (
              <div className="l2a0" style={{ background: '#fff', border: '2px dashed #ea580c', borderRadius: '16px', padding: '16px', marginBottom: '14px', textAlign: 'center' }}>
                ...
                <label htmlFor="l2-photo-upload-mobile" style={{
                  display: 'inline-block', padding: '10px 20px', background: isUploadingPhoto ? '#9ca3af' : 'linear-gradient(135deg,#ea580c,#c2410c)',
                  color: '#fff', borderRadius: '10px', fontSize: '13px', fontWeight: 600, cursor: isUploadingPhoto ? 'not-allowed' : 'pointer',
                  boxShadow: '0 4px 12px rgba(234,88,12,0.3)',
                }}>
                  {isUploadingPhoto ? '⏳ Uploading...' : '📂 Choose Photo'}
                </label>
                <input
                  id="l2-photo-upload-mobile"
                  type="file"
                  accept="image/*"
                  style={{ display: 'none' }}
          ...
          <div className="mx-auto max-w-[90%] sm:max-w-[1000px] mt-8">
            ...
            {(!userData.imageUrl || !userData.imageUrl.startsWith('http')) && (
              <div style={{ margin: '24px auto 0', maxWidth: '480px', background: 'linear-gradient(135deg,#fff7ed,#fff)', border: '2px dashed #ea580c', borderRadius: '20px', padding: '24px', textAlign: 'center' }}>
                ...
                <label htmlFor="l2-photo-upload-desktop" style={{
